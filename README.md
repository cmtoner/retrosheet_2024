...
The following sections detail the discovered errors, inconsistencies, and changes made to the source data.
...
- Line scores are reformatted to assist with splitting: parentheses are replace with the tilde (~) symbol.
...
- Dates are standardized to the YYYY-mm-dd format.
- 'Leadoff' and 'game start' columns are replaced with 'inning_progress' and 'game_progress'. These columns have values that indicate the start, middle, and end of the inning/game with the values 's', 'm', and 'f', respectively (for 'start', 'middle', 'finish').
...
| Start Time (source file) | New Value |
|--------------------------|-----------|
| 0.3125                   | Null      |
...
| **Wind Direction (source)** | **Wind Direction (new)** |
|-----------------------------|--------------------------|
| fromcf                      | from_center              |
| fromlf                      | from_left                |
| fromrf                      | from_right               |
...
Reconciling inconsistent values between the game logs, BGAME file, and the game info file: The table below indicates the changes and evidence supporting the change.
